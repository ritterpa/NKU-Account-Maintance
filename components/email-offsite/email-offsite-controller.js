app.controller('EmailOffsiteCtrl', ['$scope', '$rootScope', 'User', 'ExchangeUser', function ($scope, $rootScope, User, ExchangeUser) {
    $scope.status = 'none';
    ...
    $scope.SetUser = function (username) {
        ...
        ExchangeUser.getoffsite({ username: username }).$promise.then(function (user) {
            $scope.status = 'loaded';
            $scope.user = user;
